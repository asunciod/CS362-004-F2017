...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	{
	case adventurer:
		return adventurerCard(drawntreasure, state, currentPlayer, cardDrawn = 0, temphand, z);
	case council_room:
		//+4 Cards
		for (i = 0; i < 4; i++)
		{
	...
	case feast:
		return feastCard(i = 0, state, currentPlayer, temphand, x = 0, choice1);
	case gardens:
		return gardensCard();
	case mine:
	...
	case smithy:
		return smithyCard(i = 0, currentPlayer, state, handPos);
	case village:
		//+1 Card
		drawCard(currentPlayer, state);
